...
class mBot():
	...
	def __commsThread(self, callback):
		while 1:
			  
			if(self.exiting==True):
				break
                        if (not self.reqInProgress):
                                try:
                                        print "Getting request from queue.."
                                        self.currentRequest = reqQueue.get()
                                        self.device.writePackage(self.currentRequest.txBuffer)
                                        #__writePackage(self,pack.currentRequest.txBuffer)

                                        print self.currentRequest.txBuffer
                                        print "Data :" + self.currentRequest.txBuffer
...
#		self.device.writePackage(pack)
	...
	def __queueReadRequest(self, sensor, pack):
                if not sensor.readPending:
                        ...
                        self.reqQueue.put(request(sensor))
...
# Grid Follower Interface
	...
	def requestGridX(self,extID):
		#self.__doCallback(extID,callback)
		#self.__writePackage(bytearray([0xff,0x55,0x3,extID,0x1,0xc8]))
		self.__queueReadRequest(self.gridX, bytearray([0xff,0x55,0x3,extID,0x1,0xc8]))

	def requestGridY(self,extID):
		#self.__doCallback(extID,callback)
		#self.__writePackage(bytearray([0xff,0x55,0x3,extID,0x1,0xc9]))
		self.__queueReadRequest(self.gridY, bytearray([0xff,0x55,0x3,extID,0x1,0xc9]))		

	def doGridX(self,x):
		#self.__writePackage(bytearray([0xff,0x55,0x5,0x0,0x2,0xc8] + self.short2bytes(x)))
		self.__queueWriteRequest(self.gridX, bytearray([0xff,0x55,0x5,0x0,0x2,0xc8] + self.short2bytes(x)))
		
	def doGridY(self,y):
		#self.__writePackage(bytearray([0xff,0x55,0x5,0x0,0x2,0xc9] + self.short2bytes(y)))
